...
namespace Servicios.Persona
{
    public class Cliente : Persona
    {
        ...
        public override IEnumerable<PersonaDto> Obtener(string cadenaBuscar)
        {
            Expression<Func<Dominio.Entidades.Cliente, bool>> filtro = cliente => cliente.Apellido.Contains(cadenaBuscar)
                     || cliente.Nombre.Contains(cadenaBuscar)
                     || cliente.Dni == cadenaBuscar;

            return _unidadDeTrabajo.ClienteRepositorio.Obtener(filtro, "CondicionIva, Localidad, " +
